...
- Leisure lock should be calm and literal. It blocks spending, not saving: Daily rewards can keep accumulating, phone-use correction pauses, and interest waits until the app is restarted. Use a restart treatment that points to one uninterrupted two-hour focus and the three-hour starter reserve; do not use error color or failure language.
...
- Rules are compact stepper rows, not long forms. Daily grant, day boundary, wake time, sleep-protection start, and sleep rate are adjustable from Settings. Savings interest is a fixed product rule for now: 5% on the first 6h, 8% from 6h to 24h, and 12% above 24h.
...
Low reserve, depleted, and locked states should stay calm. Avoid shame copy, alarm language, and red warning surfaces unless an action is destructive. When reserve is high, keep the well literal and calm: show saved time and interest without extra shadow or triumphant effects.
